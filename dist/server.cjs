...
`;
      import_fs.default.appendFileSync(import_path.default.join(process.cwd(), "leads.txt"), leadEntry, "utf8");
...
`;
      ...
      if (gmailPass) {
        ...
        const formattedAnswers = answersText ? answersText.split("\n").filter((l) => l.trim()).map((line) => {
        ...
        const htmlBody = `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 24px; border: 1px solid #e0e0e0; border-radius: 12px;">
            ...
            <div style="margin-top:24px; padding:16px; background:#e8f0fe; border-radius:8px; text-align:center;">
              ...
              <p style="margin:4px 0 0; color:#6B7280; font-size:12px;">Received at ${(/* @__PURE__ */ new Date()).toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })} IST</p>
            </div>
          </div>`;
        await transporter.sendMail({
          ...
          subject: `\u{1F514} New Consultation: ${name} \u2014 ${service || "General"}`,
          html: htmlBody
        });
      } else {
